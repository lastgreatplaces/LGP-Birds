  ...
  useEffect(() => {
    async function loadPlaces() {
      ...
      if (urlLocked && urlPlaceName) {
        ...
        if (match) {
          setSelectedPlaceId(String(match.site_id))
          setLinkReady(true)
        } else {
          console.warn('No linked bird place match found for:', selectedState, urlPlaceName)
          setLinkReady(false)
        }
      } else {
        setLinkReady(true)
    ...
    loadPlaces()
  }, [selectedState, urlLocked, urlPlaceName])
  ...
  return (
    <div style={{ padding: '12px', maxWidth: '500px', margin: '0 auto', fontFamily: 'sans-serif', color: COLORS.text }}>
      ...
      <button
        ...
        disabled={loading || (urlLocked && !linkReady)}
        style={{
          width: '100%',
          padding: '15px',
