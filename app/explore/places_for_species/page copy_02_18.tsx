...
export default function SpeciesSearch() {
  ...
  const runPowerQuery = async () => {
    ...
    if (endW < startW) {
      setSearchError(`Date Range Error: "From" week (${startW}) cannot be after "To" week (${endW}).`);
      setResults([]);
      return;
    }

    // 2. BIRD SELECTION CHECK
    if (!selectedSpecies) { 
  ...
  return (
    <div style={{ padding: '10px', maxWidth: '600px', margin: '0 auto', fontFamily: 'sans-serif', backgroundColor: '#fff' }}>
      ...
      {searchError && (
        <div style={{ color: 'white', backgroundColor: '#d32f2f', padding: '12px', borderRadius: '8px', marginBottom: '10px', fontSize: '0.85rem', fontWeight: 'bold' }}>
          ⚠️ {searchError}
        </div>
      )}
        ...
        <>
          <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', background: '#eee', padding: '2px', borderRadius: '6px', flex: 1 }}>
              <button onClick={() => setSortBy('avg')} style={{ flex: 1, padding: '10px 0', borderRadius: '5px', border: 'none', fontSize: '0.75rem', fontWeight: 'bold', color: sortBy === 'avg' ? '#007bff' : '#666', backgroundColor: sortBy === 'avg' ? 'white' : 'transparent' }}>Prob.</button>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem' }}>
              <thead>
                <tr style={{ backgroundColor: '#2e4a31', color: 'white', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '10px 4px' }}>Place</th>
